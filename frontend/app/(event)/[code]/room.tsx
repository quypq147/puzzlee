...
export default function EventRoom({ eventId, eventCode }: RoomProps) {
  ...
  return (
    <div className={cn("flex flex-col h-screen overflow-hidden bg-[#f9fafb] dark:bg-[#111827] text-gray-800 dark:text-gray-200 font-sans transition-colors duration-200", isDarkMode ? 'dark' : '')}>
      ...
      <header className="bg-[#1c5b9f] text-white h-14 flex-none shadow-md z-20 relative px-4 flex items-center justify-between">
        <div className="flex items-center gap-4 w-1/4">
          ...
          <span className="font-semibold text-lg truncate hidden sm:block">Sự kiện #{eventCode}</span>
        </div>
        ...
        <div className="flex h-full items-end justify-center w-1/2 gap-1">
          <button 
            ...
            className={cn(
              ...
              activeTab === 'qa' ? "text-white" : "text-blue-200 hover:text-white"
            )}
          >
            <MessageSquare className="w-5 h-5" />
            <span className="hidden sm:inline">Hỏi đáp</span>
            {activeTab === 'qa' && <div className="absolute bottom-0 left-0 w-full h-1 bg-white rounded-t"></div>}
          </button>
          ...
          <button 
             ...
             className={cn(
              "relative px-4 sm:px-6 py-4 font-medium flex items-center gap-2 hover:bg-white/10 transition",
              activeTab === 'poll' ? "text-white" : "text-blue-200 hover:text-white"
            )}
          >
            <BarChart2 className="w-5 h-5 rotate-90" />
            <span className="hidden sm:inline">Bình chọn</span>
            {activeTab === 'poll' && <div className="absolute bottom-0 left-0 w-full h-1 bg-white rounded-t"></div>}
          </button>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <aside className="w-72 bg-white dark:bg-[#1f2937] border-r border-gray-200 dark:border-gray-700 hidden md:flex flex-col justify-between overflow-y-auto z-10">
          <div className="p-6">
            <div className="mb-8">
              <h2 className="font-bold text-lg mb-1 dark:text-white">Sự kiện Demo</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">14 - 16 thg 12, 2025</p>
              <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">#{eventCode}</p>
            </div>
            ...
            <div className="flex items-center justify-between px-2 py-2 cursor-pointer" onClick={toggleTheme}>
              ...
              {/* Toggle Switch UI */}
              <div className={cn("w-10 h-5 bg-gray-300 rounded-full relative transition-colors duration-300", isDarkMode ? "bg-[#1c5b9f]" : "")}>
                 <div className={cn("w-5 h-5 bg-white rounded-full absolute top-0 border-2 transition-all duration-300", isDarkMode ? "right-0 border-[#1c5b9f]" : "left-0 border-gray-300")}></div>
              </div>
            </div>
          </div>

          <div className="p-6 text-center">
             <div className="text-2xl font-bold tracking-tight text-[#1d8e3e] opacity-20 select-none">slido</div>
        ...
        <main className="flex-1 relative flex flex-col items-center w-full overflow-y-auto pb-20 scroll-smooth bg-[#f9fafb] dark:bg-[#111827]">
          ...
          <div className="w-full max-w-3xl px-4 flex-1">
            {questions.length === 0 ? (
              <div className="flex flex-col items-center justify-center w-full py-12 md:py-20 text-center">
                ...
                <h3 className="text-gray-500 dark:text-gray-400 text-lg mb-1 font-medium">Chưa có câu hỏi nào.</h3>
                <p className="text-gray-800 dark:text-gray-100 text-xl font-bold">Hãy là người đầu tiên đặt câu hỏi!</p>
              </div>
            ) : (
          ...
          <div className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-30">
            ...
            >
              <span className="font-bold text-lg hidden md:inline">Gửi câu hỏi</span>
